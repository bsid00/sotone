...
/// Sotone fork (2026-08-12, patch 3): the scoped variant. Under XRecord the
/// scope is the recorded *event range* — a keyboard-only listener asks the X
/// server for `KeyPress..KeyRelease` and is therefore never woken for pointer
/// motion, rather than being woken and discarding it.
///
...
/// unproven until CI builds Linux.
    ...
    T: FnMut(Event) + 'static,
{
    ...
    unsafe {
        ...
        // X11 numbers these contiguously: KeyPress, KeyRelease, ButtonPress,
        // ButtonRelease, MotionNotify. A keyboard-only scope stops the range at
        // KeyRelease, which also excludes the buttons — correct, because a
        // caller that asked for `Keyboard` has no binding a button could match.
        record_range.device_events.last = if scope.includes_mouse() {
            xlib::MotionNotify as c_uchar
        } else {
            xlib::KeyRelease as c_uchar
        };
