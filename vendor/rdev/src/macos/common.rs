...
pub enum CGEventTapOption {
    // Sotone fork: `Default = 0` (the *intercepting* tap option) is deleted with
    // the `grab` API. A listen-only tap cannot swallow or alter events.
    ListenOnly = 1,
}
...
/// Sotone fork (2026-08-12, patch 3): the keyboard half of the mask above, for
/// `HookScope::Keyboard`. A tap asks Core Graphics for the event types it wants
/// and is simply not called for the rest, so a keyboard-only listener never
/// enters the pointer-motion path at all.
///
/// **Compile-unverified**: only Windows is built here (README-SOTONE.md,
/// "Verification status").
#[allow(non_upper_case_globals)]
